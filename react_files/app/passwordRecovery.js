    ...
    const recoverPress = async () => {
        if (email === email2) {
            ...
            try {
                ...
                console.log('E-mail został wysłany:', emailResponse.data);
        
                // Aktualizacja hasła w bazie danych
                const updateResponse = await axios.put(`/user/${email}`, { password: new_password });
        
                console.log('Aktualizacja hasła w bazie danych:', updateResponse.data);
